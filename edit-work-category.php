...
<?php  
 ...
 if(isset($_REQUEST['btnedit'])){
	...
	if($procatname!=""){
		...
		//
	  $num=mysqli_num_rows(mysqli_query($url,"SELECT `cat_name` FROM `".TB_pre."brand_categories` WHERE `cat_name`='$procatname' && `parent_cat`='$parent' && cat_id!=".$cat_id));
	  //var_dump($num); exit;
	  if($num < 1){
		  $query = "UPDATE `".TB_pre."brand_categories` SET `cat_name`='$procatname',`parent_cat`='$parent', cat_desc='".$_POST['cat_desc']."', cat_imgs='$g_images', cat_videos='$cat_videos',  cat_banner='$p_image' WHERE `cat_id`=".$cat_id;
		  $r = mysqli_query($url, $query) or die(mysqli_error($url));
		  if($r){
			  $msg.= "Work Category Successfully Updated";
		  }
		  else {
			...
			$error.= "Failed: Work Category already exist";
		}
	}
}
...
$res_cat=mysqli_query($url,"SELECT * FROM `".TB_pre."brand_categories` WHERE `cat_id`=".$_GET['cat_id']);
            ...
            <div class="box-body">
              <form role="form" method="post"  class="form-horizontal" action="" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                      ...
                      <input type="text" class="form-control" placeholder="Work Category" name="productmenu" id="productmenu" value="<?php echo $row->cat_name; ?>" />
                    </div>
                      ...
                      <?php
					  $r_p=mysqli_query($url,"select * from `".TB_pre."brand_categories` WHERE parent_cat=0 && cat_id!=".$_GET['cat_id']);
					 ...
					 <?php
                      ...
                      <textarea class="form-control" placeholder="Workd Category" name="cat_desc" id="catdesc"><?php echo $row->cat_desc; ?></textarea>
                    </div>
