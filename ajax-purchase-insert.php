...
$query2 = "INSERT INTO ".TB_pre."purchase (`purchase_no`,`product_id`,`quantity`,`purchased_price`,`currency`,`supplier_id`) VALUES(?,?,?,?,?,?);";
...
for($count = 0; $count<count($_POST['hidden_product_name']); $count++)
{
    ...
    /* https://www.php.net/manual/en/mysqli-stmt.bind-param.php */
    $statement->bind_param('isddii', $data[':purchaseno'], $data[':product_name'], $data[':product_stock'], $data[':original_price'], $data[':currency_name'], $data[':supplier_name']);
    $statement->execute();
