...
<table style="font-family: Arial, Helvetica, sans-serif;" width="750" align="center">
...
<td>			  
<table width="100%">
	<tbody align="right" style="float: right;">
<tr>
  ...
  <td>&nbsp;</td>
	...
	<tbody>
		<?php 
			...
			$base_price=round($res_purmast->conversion_ratio*$res->purchased_price, 2);
			$shipping=round(($base_price*$res->quantity)*($res_pur->shipping/100)/$res->quantity, 2);
			$customs=round((($base_price*$res->quantity)*($res_pur->customs/100))/$res->quantity, 2);
			$handling=round((($base_price*$res->quantity)*($res_pur->handling/100))/$res->quantity, 2);
			/*$shipping=round(($base_price*$res->quantity)*($res_pur->shipping/100))/$res->quantity;
			$customs=round((($base_price*$res->quantity)*($res_pur->customs/100))/$res->quantity, 2);
			$handling=round((($base_price*$res->quantity)*($res_pur->handling/100))/$res->quantity, 2);*/
			$unit_price=$base_price+$shipping+$customs+$handling;
...
<?php } ?>
		...
		<th><strong style="font-size: 18px;">Grand Total</strong></th>
...
<input type='button' id='btn' value='Print' onclick='printDiv();'>	
	...
	<button style="margin-top: 50px;"><a style="text-decoration: none; color: #828282;" href="purchase_report.php?pno=<?php  echo $purchaseno; ?>">Go Back</a></button>
</div>
</center>
