...
if(isset($_GET['remove_cat'])){
	$id = $_GET['remove_cat'];
	$query = "DELETE FROM `".TB_pre."pcategories` WHERE `cat_id`='$id'";
...
$sql="select * from `".TB_pre."pcategories` ORDER BY parent_cat";
...
?>  
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            <li><a href="add-category.php" class="btn btn-block"><i class="fa fa-plus"></i> Add Product Category</a></li>
          </ol>
                  ...
                  <table id="example2" class="table table-bordered table-hover center-table" style="width: 100%; max-width: 750px; float: left;">
					...
					while($res = mysqli_fetch_array($r1)){ ?>
                      <tr>
                        ...
                        <?php
                        if($res['parent_cat']!=0){	
		   					echo mysqli_fetch_object(mysqli_query($url,"select cat_name from `".TB_pre."pcategories` WHERE `cat_id`=".$res['parent_cat']))->cat_name; 
						}
						else echo "Default";
						?>
                        </td>
                        
                        <td><a href="edit-category.php?cat_id=<?php echo $res['cat_id']; ?>" class="btn btn-primary" style="width:100px;" title="">Edit</a></td>
   ...
   <?php include_once('includes/footer-scripts.php'); ?>
    ...
    function removeItem(id){
		...
		if(c){
			location = "categories.php?remove_cat="+id;
		}
	}
